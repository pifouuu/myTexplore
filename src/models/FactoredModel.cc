...
FactoredModel::FactoredModel(const FactoredModel & m) :
	...
	if (COPYDEBUG)
		cout << "MDP Tree copy constructor" << endl;
	...
	if (m.outputModels.size() > 0) {
		if (COPYDEBUG)
			cout << " FactoredModel copy trees" << endl;
		rewardModel = m.rewardModel->getCopy();
		if (m.terminalModel != NULL)
			terminalModel = m.terminalModel->getCopy();
		if (COPYDEBUG)
			cout << " copy output trees" << endl;
		outputModels.resize(m.outputModels.size());
		for (unsigned i = 0; i < m.outputModels.size(); i++) {
			outputModels[i] = m.outputModels[i]->getCopy();
		}
		if (COPYDEBUG)
			cout << " FactoredModel trees copied" << endl;
	}
	if (COPYDEBUG)
		cout << "FactoredModel copy complete " << endl;
}
...
bool FactoredModel::initMDPModel(int nfactors) {
	if (MODEL_DEBUG)
		cout << "Init trees for each state factor and reward" << endl;

	outputModels.resize(nfactors);

	bool simpleRegress = false;
	if (modelType == M5SINGLE || modelType == M5ALLSINGLE
			|| modelType == LSTSINGLE)
		simpleRegress = true;
	...
	for (int i = 0; i < nfactors; i++) {
		if (modelType == C45TREE && nModels == 1) {
			outputModels[i] = new C45Tree((id * (nfactors + 1)) + i,
					treeBuildType, 5, M, 0, rng);
			if (i == 0) {
				rewardModel = new C45Tree((id * (nfactors + 1)) + nfactors,
						treeBuildType, 5, M, 0, rng);
				if (episodic)
					terminalModel = new C45Tree(
							(id * (nfactors + 1)) + nfactors + 1, treeBuildType,
							5, M, 0, rng);
			}
		} else if ((modelType == M5MULTI || modelType == M5SINGLE)
				&& nModels == 1) {
			outputModels[i] = new M5Tree((id * (nfactors + 1)) + i,
					treeBuildType, 5, M, 0, simpleRegress, false,
					treeThresh * featRange[i], rng);
			if (i == 0) {
				rewardModel = new M5Tree((id * (nfactors + 1)) + nfactors,
						treeBuildType, 5, M, 0, simpleRegress, false,
						treeThresh * rRange, rng);
				if (episodic)
					terminalModel = new M5Tree(
							(id * (nfactors + 1)) + 1 + nfactors, treeBuildType,
							5, M, 0, simpleRegress, false, treeThresh, rng);
			}
		} else if ((modelType == M5ALLMULTI || modelType == M5ALLSINGLE)
				&& nModels == 1) {
			outputModels[i] = new M5Tree((id * (nfactors + 1)) + i,
					treeBuildType, 5, M, 0, simpleRegress, true,
					treeThresh * featRange[i], rng);
			if (i == 0) {
				rewardModel = new M5Tree((id * (nfactors + 1)) + nfactors,
						treeBuildType, 5, M, 0, simpleRegress, true,
				...
				if (episodic)
					terminalModel = new M5Tree(
							(id * (nfactors + 1)) + 1 + nfactors, treeBuildType,
							5, M, 0, simpleRegress, true, treeThresh, rng);
			}
		} else if ((modelType == LSTMULTI || modelType == LSTSINGLE)
				&& nModels == 1) {
			outputModels[i] = new LinearSplitsTree((id * (nfactors + 1)) + i,
					treeBuildType, 5, M, 0, simpleRegress,
					treeThresh * featRange[i], rng);
			if (i == 0) {
				rewardModel = new LinearSplitsTree(
						(id * (nfactors + 1)) + nfactors, treeBuildType, 5, M,
						0, simpleRegress, treeThresh * rRange, rng);
				if (episodic)
					terminalModel = new LinearSplitsTree(
							(id * (nfactors + 1)) + 1 + nfactors, treeBuildType,
							5, M, 0, simpleRegress, treeThresh, rng);
			}
		} else if (modelType == STUMP && nModels == 1) {
			outputModels[i] = new Stump((id * (nfactors + 1)) + i, 1, 5, M, 0,
					rng);
			if (i == 0) {
				rewardModel = new Stump((id * (nfactors + 1)) + nfactors, 1, 5,
						M, 0, rng);
				if (episodic)
					terminalModel = new Stump(
							(id * (nfactors + 1)) + 1 + nfactors, 1, 5, M, 0,
							rng);
			}
		} else if (predType == SEPARATE && nModels > 1) {
			outputModels[i] = new SepPlanExplore((id * (nfactors + 1)) + i,
					modelType, predType, nModels, treeBuildType, 5, FEAT_PCT,
					EXP_PCT, treeThresh * featRange[i], stoch, featRange[i],
					rng);
			if (i == 0) {
				rewardModel = new SepPlanExplore(
						(id * (nfactors + 1)) + nfactors, modelType, predType,
						nModels, treeBuildType, 5, FEAT_PCT, // remove this pct of feats
						EXP_PCT, treeThresh * rRange, stoch, rRange, rng);
				if (episodic) {
					terminalModel = new SepPlanExplore(
							(id * (nfactors + 1)) + 1 + nfactors, modelType,
							predType, nModels, treeBuildType, 5, FEAT_PCT, // remove this pct of feats
			...
			outputModels[i] = new MultipleClassifiers((id * (nfactors + 1)) + i,
					...
					rng);
			if (i == 0) {
				rewardModel = new MultipleClassifiers(
						(id * (nfactors + 1)) + nfactors, modelType, predType,
						nModels, treeBuildType, 5, FEAT_PCT, // remove this pct of feats
						EXP_PCT, treeThresh * rRange, stoch, rRange, rng);
				if (episodic) {
					terminalModel = new MultipleClassifiers(
							(id * (nfactors + 1)) + 1 + nfactors, modelType,
							predType, nModels, treeBuildType, 5, FEAT_PCT, // remove this pct of feats
							EXP_PCT, treeThresh, stoch, 1.0, rng);
				}
...
bool FactoredModel::updateWithExperiences(std::vector<experience> &instances) {
	if (MODEL_DEBUG)
		cout << "FactoredModel updateWithExperiences : " << instances.size()
				<< endl;
	...
	if (outputModels.size() != instances[0].next.size()) {
		if (MODEL_DEBUG)
			cout << "ERROR: size mismatch between input vector and # trees "
					<< outputModels.size() << ", " << instances[0].next.size()
					<< endl;
		return false;
		exit(-1);
	}
...
bool FactoredModel::updateWithExperience(experience &e) {
	if (MODEL_DEBUG)
		cout << "updateWithExperience " << &(e.s) << ", " << e.act << ", "
	...
	if (MODEL_DEBUG) {
		...
		cout << "Reward: " << e.reward << " term: " << e.terminal << endl;
	}
	...
	if (MODEL_DEBUG)
		cout << "Model updated, changed: " << changed << endl;
	return changed;
...
float FactoredModel::getStateActionInfo(const std::vector<float> &state,
	...
	if (MODEL_DEBUG) {
		...
		//    cout << endl;
		cout << "a: " << act << " has " << retval->transitionProbs.size()
				<< " outcomes already" << endl;
	}
	...
	if (MODEL_DEBUG)
		cout << "avg conf returned " << confSum << endl;
...
void FactoredModel::addFactorProb(float* probs, std::vector<float>* next,
		std::vector<float> x, StateActionInfo* retval, int index,
		std::vector<std::map<float, float> > predictions, float* confSum) {
	...
	for (std::map<float, float>::iterator it1 = outputPreds.begin();
		...
		if (MODEL_DEBUG)
			cout << "Prob of outcome " << val << " on factor " << index
					<< " is " << (*it1).second << endl;

		// ignore it if it has prob 0
		if ((*it1).second == 0) {
			if (MODEL_DEBUG)
				cout << "Prob 0, ignore" << endl;
			continue;
		}
